...
function AppreciationBoard() {
  ...
  return <div className="landing-board-wrap">
    <div className="landing-board" aria-label="Example TipWall appreciation board">
      <div className="landing-board-strip"><span><i className="landing-live-dot" /> LIVE WALL</span><span>maya.builds <b>↗</b></span></div>
      <div className="board-title">People leave a little<br /><em>something behind.</em></div>
      {notes.map((note) => <article key={note.from} className="landing-note" style={{ '--note-top': note.top, '--note-left': note.left, '--note-rotate': note.rotate, '--note-color': note.color } as React.CSSProperties}>
        <div className="landing-tape" />
        <p>{note.text}</p>
        <footer><span>{note.from} · {note.reason}</span><strong>{note.amount}</strong></footer>
      </article>)}
      <div className="landing-board-mark"><Image src="/logo.svg" alt="" width={30} height={30} /> TipWall</div>
    </div>
    <div className="landing-pin-form">
      <label htmlFor="demo-note">Leave a note</label>
      ...
      <div className="landing-pin-row"><span>{message.length}/90</span><button type="button" onClick={() => message.trim() && setPosted(true)}>{posted ? 'Pinned ✓' : 'Pin a note →'}</button></div>
    </div>
  </div>
}
...
export default function HomePage() {
  ...
  const [displayName, setDisplayName] = useState(''); const [bio, setBio] = useState(''); const [achievement, setAchievement] = useState('')
  ...
  const submit = async (e: React.FormEvent) => { e.preventDefault(); setError(null); const handleStr = normalizeHandle(handle); if (handleStr.length < 3) return setError('Handle must be at least 3 characters'); if (!wallet) return setError('Connect your Nimiq wallet first'); setSubmitting(true); try { const auth = await signProfileAuth({ action: 'create', handle: handleStr, walletAddress: wallet }); const res = await fetch('/api/profile/create', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ handle: handleStr, walletAddress: wallet, displayName: displayName || handleStr, bio, achievement: achievement || undefined, auth }) }); const data = await res.json(); if (!res.ok) throw new Error(data.error); window.location.href = `/${data.handle}/share?new=1` } catch (err) { setError(err instanceof Error ? err.message : 'Failed to create profile'); setSubmitting(false) } }
  const normalizedHandle = normalizeHandle(handle)
  return <main className="landing-page">
    <header className="landing-header"><Link href="/" className="landing-logo"><Image src="/logo.svg" alt="TipWall logo" width={38} height={38} /><span>TipWall</span></Link><nav><Link href="/explore">Explore creators</Link><button onClick={() => setupRef.current?.scrollIntoView({ behavior: 'smooth' })}>Create a wall <span>↗</span></button></nav></header>
    <section className="landing-hero"><div className="landing-hero-copy"><p className="landing-eyebrow"><i className="landing-live-dot" /> SUPPORT THAT SAYS SOMETHING</p><h1>Every fan<br />leaves a <em>mark.</em></h1><p className="landing-lede">Tip creators directly in NIM, and leave the reason it mattered. A public wall of support, insight, and tiny moments that last.</p><div className="landing-actions"><button className="landing-btn landing-btn-dark" onClick={() => setupRef.current?.scrollIntoView({ behavior: 'smooth' })}>Make your wall <span>↗</span></button><Link className="landing-btn landing-btn-light" href="/explore">Explore walls <span>→</span></Link></div><div className="landing-trust"><span>0% platform fee</span><span>Direct to wallet</span><span>On-chain verified</span></div></div><AppreciationBoard /></section>
    <div className="landing-ticker"><span>TIP THE CREATOR · SAY WHY IT MATTERED · LEAVE A MARK · </span><span>TIP THE CREATOR · SAY WHY IT MATTERED · LEAVE A MARK · </span></div>
